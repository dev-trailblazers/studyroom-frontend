...
const Select = ({ options, placeholder, value, setValue }: SelectProps) => {
  return (
    <CustomSelect
      ...
      labelProps={{
        className:
          'pl-5 pt-1 text-gray_DD text-[12px] peer-aria-expanded:pt-0 peer-data-[value=true]:pt-0 before:absolute before:left-0 before:top-[-0.5px] before:border-l before:border-gray_DD before:border-t before:rounded-tl-lg before:border-solid  after:border-r after:border-gray_DD after:w-[220px] after:border-t after:rounded-tr-lg after:border-solid after:absolute after:right-0 after:top-[-0.5px]',
      }}
    >
      {options.map((opt) => (
        <Option
