...
const Notice = () => {
  ...
  const [headerActiveIndex, setHeaderActiveIndex] = useState(1);
  ...
  return (
    <>
      <div className="min-h-screen pt-10 pl-10 pr-[50px] lg:pl-20 lg:pr-[90px] flex flex-col gap-10">
        ...
        {/* Notice Section*/}
        <div className="p-5 flex flex-col rounded-[15px] bg-blue_04 shadow-box_03">
          <div className="grid grid-cols-1 gap-5">
            <span className="text-[24px] font-semibold text-blue_01">
              공지사항
            </span>
            {currentItems.map((notice, index) => (
              <div
                key={notice.id}
                className="w-full p-5 min-h-[60px] bg-white shadow-box_01 font-semibold rounded-[5px] cursor-pointer"
                onClick={() => {
          ...
          <div className="flex justify-center mt-4">
            ...
            {Array.from(
              ...
              (_, i) => (
                <button
                  key={i}
                  className={`mx-1 px-3 py-1 rounded-lg ${noticePage === i + 1 ? 'bg-main text-white shadow-box_03' : 'bg-white shadow-box_01'}`}
